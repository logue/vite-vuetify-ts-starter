...
Remove text decoration with the `.text-decoration-none` class or add an _overline, underline or line-through_ by using `.text-decoration-overline`, `.text-decoration-underline`, and `.text-decoration-line-through`.
...
```scss { resource="src/styles/settings.scss" }
@use 'vuetify/settings' with (
  $body-font-family: var(--font-sans) // $heading-font-family inherits the same font in this example
);
```
