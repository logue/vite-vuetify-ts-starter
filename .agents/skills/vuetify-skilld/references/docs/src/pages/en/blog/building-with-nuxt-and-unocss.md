...
export default defineNuxtConfig({
  ...
  unocss: {
    ...
    safelist: [
      /* classes that have to be always generated */
    ]
  }
});
```
...
@use 'vuetify/settings' with (
  $color-pack: false,
  // <- disable colors from Material palette
  $utilities: false
);
```
...
```html
<v-container class="h-full flex items-center" max-width="900"></v-container>
```
...
export const forTailwind = Object.entries(breakpoints).reduce(
...
<template>
  <v-container ref="container" class="mt-12 outline-dashed">
    <v-sheet class="mx-auto pa-6" max-width="400">
      ...
      <br />
      <v-row wrap dense>
        <v-col v-for="i in 12" :key="i" cols="12" sm="6" md="4" lg="3" xl="2" xxl="1">
          <div
            class="
              bg-red-500/50
              sm:bg-lime-500/50
...
@use 'vuetify/settings' with (
  $layers: true,
  // <- enable layers for CSS generated with Sass
   // ...
);
```
