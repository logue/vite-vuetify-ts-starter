...
@use 'vuetify/settings' with (; // variables go here);
```
...
Import order of stylesheets becomes much more important with layers enabled, `import 'vuetify/styles'` or a file containing `@use 'vuetify'` **must** be loaded _before_ any components or the CSS reset will take precedence over component styles and break everything.
